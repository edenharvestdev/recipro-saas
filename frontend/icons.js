/* ============================================================================
 * Recipro — Inline SVG Icon System
 * ----------------------------------------------------------------------------
 * Pure vanilla JS, no dependencies. Uses real Lucide (ISC/MIT-licensed) path
 * data. Exposes:
 *   - window.icon(name, opts)   -> SVG string
 *   - window.renderIcons(root)  -> hydrates [data-icon] elements (idempotent)
 *
 * Every <svg> uses stroke:currentColor so icons inherit text color, and is
 * sized in em (default 1em) so they inherit the surrounding font-size.
 * ==========================================================================*/
(function () {
  "use strict";
  ...
  /* --------------------------------------------------------------------------
   * Icon body registry.
   * Each value is the INNER markup of the <svg> (paths / circles / lines etc.)
   * copied from Lucide. The outer <svg> wrapper (viewBox, stroke attrs, size)
   * is added by icon().
   * ------------------------------------------------------------------------*/
  var ICONS = {
    /* ---- Sidebar / app chrome ------------------------------------------- */
    ...
    "crown":
      '<path d="M11.562 3.266a.5.5 0 0 1 .876 0L15.39 8.87a1 1 0 0 0 1.516.294L21.183 5.5a.5.5 0 0 1 .798.519l-2.834 10.246a1 1 0 0 1-.956.734H5.81a1 1 0 0 1-.957-.734L2.02 6.02a.5.5 0 0 1 .798-.52l4.276 3.664a1 1 0 0 0 1.516-.294z"/><path d="M5 21h14"/>',
    "shield":
    ...
    /* alias requested by chrome list */
    "check-circle":
      '<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>',
    "circle-x":
    ...
    /* ---- Added: previously-missing referenced icons --------------------- */
    "bar-chart-2":
    ...
    "calendar-range":
      '<rect width="18" height="18" x="3" y="4" rx="2"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M3 10h18"/><path d="M17 14h-6"/><path d="M13 18H7"/><path d="M7 14h.01"/><path d="M17 18h.01"/>',
    "sun":
    ...
    "package-plus":
      '<path d="M16 16h6"/><path d="M19 13v6"/><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l2-1.15"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>',
    "arrow-down-to-line":
  ...
  var FALLBACK = "help-circle";

  /* Escape attribute values to keep output well-formed even with odd input. */
  function esc(s) {
   ...
   *   opts.size        -> CSS length applied to width/height (e.g. "1.25em",
   *                       "20px", or a number treated as px). Default "1em".
   *   opts.cls         -> extra class names appended to "icon".
   *   opts.strokeWidth -> numeric stroke width (default 2).
   * ------------------------------------------------------------------------*/
  function icon(name, opts) {
    opts = opts || {};
    var key = (name == null ? "" : String(name)).trim();
    var body = Object.prototype.hasOwnProperty.call(ICONS, key)
      ? ICONS[key]
      : ICONS[FALLBACK];

    var size = opts.size;
    if (size == null || size === "") size = "1em";
    else if (typeof size === "number") size = size + "px";
    size = esc(size);

    var sw = opts.strokeWidth;
    if (sw == null || sw === "") sw = 2;

    var cls = "icon" + (opts.cls ? " " + esc(opts.cls) : "");

    return (
      '<svg class="' + cls + '" viewBox="0 0 24 24" fill="none" ' +
      'stroke="currentColor" stroke-width="' + esc(sw) + '" ' +
  ...
  /* --------------------------------------------------------------------------
   * renderIcons(root) -> hydrate every [data-icon] under root.
   *   Reads:  data-icon       -> icon name
   *           data-icon-size  -> optional size (e.g. "1.25em", "20px")
   *           data-icon-class -> optional extra classes
   *   Idempotent: marks hydrated elements so re-running is cheap and safe.
   *   Re-renders if the requested name/size/class changed since last run.
   * ------------------------------------------------------------------------*/
  function renderIcons(root) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var sig = name + "|" + size + "|" + cls;
      if (el.getAttribute("data-icon-rendered") === sig) continue; // idempotent
      el.innerHTML = icon(name, { size: size, cls: cls });
      el.setAttribute("data-icon-rendered", sig);
    }
    return root;
  }
  ...
  /* Auto-hydrate any static [data-icon] markup once the DOM is ready, so
     the file works even if index.html never calls renderIcons() explicitly. */
