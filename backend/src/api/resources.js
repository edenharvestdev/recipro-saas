...
// A0: destructive deletes require permission (was previously ungated → any staff could delete).
// recipes → recipe_edit; bills(draft) → bill_edit_draft. materials/suppliers have no dedicated
// edit permission in the catalog yet, so — per Founder STEP 3 — they are conservatively OWNER-ONLY.
// TODO(Track A): introduce material_edit / supplier_edit permissions and relax OWNER-ONLY here.
const DELETE_RULE = { suppliers: 'OWNER', materials: 'OWNER', recipes: 'recipe_edit', bills: 'bill_edit_draft' };

for (const table of TABLES) {
  router.delete(`/${table}/:id`, async (req, res) => {
    try {
      const rule = DELETE_RULE[table];
      const allowed = rule === 'OWNER' ? (req.role === 'owner' || req.isSuperadmin === true) : req.hasPerm(rule);
      if (!allowed) {
