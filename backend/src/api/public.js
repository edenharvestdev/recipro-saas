...
async function shopByToken(token) {
  ...
  const r = await query(
    `select s.id, s.name, ss.promptpay, ss.logo_url, coalesce(ss.order_payment_mode,'postpay') as order_payment_mode, coalesce(ss.menu_config,'{}'::jsonb) as menu_config from shops s
