// Seeder ข้อมูลจริงของ Merry Jane (แปลง id สั้น -> UUID + remap ความเชื่อมโยง)
// ใช้ได้ 2 ทาง: (1) CLI: `node scripts/import-merryjane.js`  (2) import { seedMerryJane } จาก bootstrap
...
function loadData() {
  return JSON.parse(fs.readFileSync(path.join(__dirname, 'merryjane-data.json'), 'utf8'));
}

// สร้าง/อัปเดตร้าน Merry Jane + วัตถุดิบ/สูตร/ตั้งค่า ภายใน client/transaction ที่ส่งมา
async function seedMerryJane(c, data = loadData()) {
  ...
  // ล้างข้อมูลเดิมของร้านนี้ก่อน (idempotent) — recipe_items ลบตาม cascade
  ...
  for (const r of data.recipes || []) {
    ...
    for (const it of r.items || []) {
      ...
      if (!matId) continue;
      await c.query('insert into recipe_items (recipe_id, material_id, amount) values ($1,$2,$3)', [rid, matId, it.amount]);
      itemCount++;
    }
  }
  ...
  await c.query(
    `insert into shop_settings (shop_id, phone, tax_id, address, bank, account, holder, promptpay, theme)
     values ($1,$2,$3,$4,$5,$6,$7,$8,'recipro')
  ...
  return {
    ...
    ownerEmail: OWNER_EMAIL, ownerPassword: OWNER_PASSWORD,
  };
}
...
if (require.main === module) {
  (async () => {
    try {
      const summary = await tx((c) => seedMerryJane(c));
...
module.exports = { seedMerryJane, loadData, OWNER_EMAIL, OWNER_PASSWORD };
